...
void *task_random_values(void *arg)
{
    dmot_ui_equalizer_permit_rendering();
    for (int i = 0; i < default_cycles; ++i)
    {
        for (int chan = 1; chan <= channels; ++chan)
        {
            ...
            dmot_ui_equalizer_set_channel_input_value(chan, input_dbm);
        }
        sleep_ms(default_sleep_ms);
    }
    dmot_ui_equalizer_forbid_rendering();
}
...
{
    fputs("\n\n[console_details]\n", stdout);
    const dmot_ui_screen_size c_screen = dmot_ui_get_console_size();
...
void example_equalizer_raw()
{
    const dmot_ui_screen_size c_screen = dmot_ui_get_console_size();
    fputs("\n\n[equalizer_raw]\n", stdout);
    dmot_ui_equalizer_init();
    dmot_ui_equalizer_set_width((int)(c_screen.cols * 0.8));
    dmot_ui_equalizer_permit_rendering();
    pthread_t thread;
    pthread_create(&thread, NULL, task_sine_values, NULL);
    dmot_ui_equalizer_render(stdout, true);
}
...
void example_equalizer_smoothed()
{
    ...
    dmot_ui_equalizer_render_smoothed(stdout, true);
}
