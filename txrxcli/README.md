...
## What lives here
- `linux/`: C++ dual-mode client/server binary (raw TCP, API-key gated). Server mode is working; client mode is still TODO.
...
- Tests (require `libgtest-dev`, `libspdlog-dev`, `libfmt-dev`): `make -C linux test`  
  Tests use `socketpair` and may not run in heavily restricted sandboxes.
